...
function initializeMap() {
    ...
    // FORCE THE PANE TO USE SVG RENDERER ENGINE
    ...
    // This explicitly tells Leaflet to draw geometry inside this pane using clear vectors
    map.getPane('investigationPane').appendChild(document.createElementNS("http://www.w3.org/2000/svg", "svg"));
    ...
    // This punches through the flat background canvas layout stack completely
    map.createPane('investigationPane');
...
function updateStatistics(data) {
    ...
    data.forEach(function(row) {
        ...
        // Prevent mathematical replication compounding if data spans multiple rows for the same entity
    ...
    // RETAINED EXACT POPULATION AND DASHBOARD PERCENTAGE SYMBOLS
...
function updateTopMunicipalities(data) {
    ...
    // RETAINED UPPER MUNICIPALITIES LAYOUT ARRAYS & SELECTION LIMITERS
...
function drawHeatmap(data) {
    ...
    data.forEach(function(row) {
        ...
        const marker = L.circleMarker([lat, lng], {
            ...
            fillOpacity: 0.22,      // Boosted opacity slightly to show bright and clear
            dashArray: "6, 6",      
            pane: 'investigationPane' // FORCES IT TO RENDER OVER THE CANVAS ELEMENT
        });

        // Stabilized hover action listeners
        marker.on('mouseover', function(e) {
            if (!map.hasLayer(this.investigationZone)) {
