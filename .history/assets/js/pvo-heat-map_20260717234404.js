    ...
    try {
        ...
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const population = Number(data.totalPopulation || data.Population || data.total_population) || 0;
            ...
            console.log(`⚡ Live Sync UI Push: ${totalHumanPopulation.toLocaleString()}`);

            // 🎯 SUPER DEFENSIVE DOM TARGETING
            // This loops through every possible ID configuration your CRIS dashboard might be using
            const targetIDs = ["humanPopulation", "human-population", "totalPopulation", "pop-count"];
            let elementFound = false;

            for (const id of targetIDs) {
                const card = document.getElementById(id);
                if (card) {
                    card.textContent = totalHumanPopulation > 0 ? totalHumanPopulation.toLocaleString() : "0";
                    elementFound = true;
                    console.log(`🎯 Successfully injected population into HTML element with ID: #${id}`);
                    break;
                }
            }

            if (!elementFound) {
                console.error("❌ CRIS UI Error: Could not find any HTML element matching 'humanPopulation' or its variations on this page.");
            }
            
            // Trigger your metrics calculations
        ...
        console.error("❌ Error running live population sync:", error);
            ...
            // Force update UI metrics instantly
            if (populationCard) {
                ...
                if (fallbackCard) fallbackCard.textContent = totalHumanPopulation > 0 ? totalHumanPopulation.toLocaleString() : "0";
            }
            
            // Re-render rate computations instantly if data is active
            refreshMap();
        });
...
// ==========================================================
// DYNAMIC STATISTICS & RATIO ENGINE (FIXED & ALIGNED)
// ==========================================================
...
function updateStatistics(data) {
    ...
    if (municipalityFilter && municipalityFilter.value !== "All") {
        ...
        // Sum population metrics matching the filtered subset
        trackedMunicipalitiesInFilter.forEach(mun => {
            activePopulation += municipalityPopulations[mun] || 0;
        });
    }

    // FALLBACK: If no specific municipality is filtered, or if the case log subset 
    // sums to 0, default directly to the total global human population loaded from Firestore
    if (activePopulation === 0 || (municipalityFilter && municipalityFilter.value === "All")) {
        activePopulation = totalHumanPopulation;
    }

    // Render Human Population Card instantly to the verified DOM ID
...
// UPDATE TOP MUNICIPALITIES (FIXED KEY ASSIGNMENT BUG)
function updateTopMunicipalities(data) {
    ...
    data.forEach(function(row) {
        ...
        if (!totals[municipality]) {
            totals[municipality] = 0; // Fixed: altered from broken totals[totals] loop
        }
    ...
    ranking.slice(0, 5).forEach(function(item, index) {
        ...
        div.innerHTML = `
            <span>${index + 1}. ${item.municipality}</span>
