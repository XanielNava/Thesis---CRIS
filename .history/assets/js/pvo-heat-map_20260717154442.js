// ==========================================================
// CRIS - PVO HEATMAP SYSTEM (INTEGRATED WITH IMPORT_HISTORY)
// ==========================================================

// 1. Import Firestore DB Instance and Methods
import { db, collection, getDocs, query, orderBy, limit } from '../../settings/js/settings-firebase.js';
...
// Population Database extracted from the imported history
let populationMap = {}; // Key: Municipality Name (lowercase) -> Value: Population Number
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Automatically load the latest data from the import history
    await fetchLatestImportFromFirestore();
});
...
// ==========================================================
// FIRESTORE IMPORT_HISTORY INTEGRATION
// ==========================================================

async function fetchLatestImportFromFirestore() {
    try {
        console.log("🔄 Fetching the latest uploaded dataset from import_history...");
        
        // Query to get the absolute latest import document (ordered by timestamp descending)
        const q = query(
        ...
        if (querySnapshot.empty) {
            console.warn("⚠️ No records found in import_history yet.");
            return;
        }

        const latestDoc = querySnapshot.docs[0].data();
        console.log("✅ Latest import document retrieved:", latestDoc);

        // 1. Extract and map human population from the imported file details
        // Adjust these field names to match exactly how you write them in your settings upload script!
        if (latestDoc.populationData && Array.isArray(latestDoc.populationData)) {
            populationMap = {};
            latestDoc.populationData.forEach(row => {
                const munName = (row.municipality || row.Municipality || "").trim().toLowerCase();
                const popValue = Number(row.population || row.Population) || 0;
                if (munName) {
                    populationMap[munName] = popValue;
                }
            });
            console.log("👥 Human Population census successfully mapped from import history.");
        }

        // 2. Extract and load the case dataset if it was bundled in the import
        if (latestDoc.casesData && Array.isArray(latestDoc.casesData)) {
            excelData = latestDoc.casesData;
            normalizeHeaders();
            populateFilters();
            refreshMap();
            console.log(`📊 Loaded ${excelData.length} case rows directly from import history.`);
        } else if (latestDoc.fileData && Array.isArray(latestDoc.fileData)) {
            // Alternate fallback key if you store the raw array under 'fileData'
            excelData = latestDoc.fileData;
            normalizeHeaders();
            populateFilters();
            refreshMap();
        }
        ...
        console.error("❌ Error loading dataset from import_history:", error);
    }
}
...
function updateStatistics(data) {
    ...
    // Compute total active population coverage based on loaded/filtered municipalities
    let activePopulation = 0;
    const trackedMunicipalitiesInFilter = new Set();
    ...
    data.forEach(function (row) {
        ...
        if (row.Municipality) {
            const normalizedMun = row.Municipality.trim().toLowerCase();
            trackedMunicipalitiesInFilter.add(normalizedMun);
        }
    });

    // Sum up the live population from Firestore's import_history
    trackedMunicipalitiesInFilter.forEach(munName => {
        if (populationMap[munName]) {
            activePopulation += populationMap[munName];
        } else {
            // Standard fallback if database mapping is missing a specific municipal entity
            activePopulation += 50000; 
        }
    });

    // Fallback if no data is loaded or matches
    if (activePopulation === 0) {
        // Fallback to average regional Iloilo province demographic size (~2M)
        activePopulation = 2000000; 
    }
...
function drawHeatmap(data) {
    ...
    const selected = layerFilter.value;
    if (selected === "bite") {
