...
async function fetchPopulationDataFromFirestore() {
    try {
        ...
        // Let's target the collection directly using the local window instance if needed,
        // but let's force a hard layout console dump
        ...
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            if (querySnapshot.empty) {
                console.warn("⚠️ Firestore says 'population_data' collection is completely EMPTY inside this module context.");
                return;
            }
            ...
            querySnapshot.forEach((doc) => {
                const data = doc.data();
                console.log(`Document [${doc.id}] raw data:`, data); // 🌟 Direct dump of what Firestore is handing back
            ...
            console.log("➡️ Calculated Total Human Population:", totalHumanPopulation);

            // Force override directly into the element, bypassing everything else
            const card = document.getElementById("humanPopulation");
            if (card) {
                card.innerHTML = totalHumanPopulation.toLocaleString();
                console.log("🎯 DOM INJECTION FORCED. Inner HTML set to:", card.innerHTML);
            } else {
                console.error("❌ Could not find element with id='humanPopulation' during direct injection.");
            }
        });
    } catch (error) {
        console.error("❌ Direct Sync Loop crashed:", error);
    }
}
