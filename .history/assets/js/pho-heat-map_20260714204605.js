/**
 * CRIS - PHO Heatmap Dashboard Handler Engine (ENHANCED & HARDENED)
 ...
 * transforms it to municipality-level geospatial data, and provides
 * robust fallback mechanics if standard column matching fails.
 */
...
// ==============================================================================
// HARDENED MULTI-FORMAT PARSER WITH DIAGNOSTICS
// ==============================================================================
function uploadExcel(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Convert to JSON using raw headers
            ...
            console.group("🔍 CRIS Parser Diagnostic Logs");
            console.log("📄 Sheet Name:", firstSheet);
            console.log("📊 Total Detected Rows:", rawData.length);
            console.log("🔑 Detected Raw Headers:", originalKeys);
            console.log("🧹 Normalized Lowercase Headers:", cleanKeys);
            console.log("📌 Sample Row Data:", firstRow);
            console.groupEnd();
            // ---------------------------------

            // Flexible structural footprint flags
            const hasABTC = cleanKeys.some(col => col.includes('abtc') || col.includes('facility') || col.includes('treatment'));
            const hasMunicipality = cleanKeys.some(col => col.includes('municipality') || col.includes('municipalities') || col.includes('town') || col.includes('muni'));
            ...
            if (hasABTC && !hasLatitude) {
                console.log("👉 Match Found: PHO ABTC raw facility format detected. Initiating mapping transformer...");
                excelData = transformPHOABTCFormat(rawData, originalKeys);
            } else if (hasMunicipality && hasLatitude && hasLongitude) {
                console.log("👉 Match Found: Geo-spatial standard format detected. Loading directly...");
                excelData = rawData;
            } else {
                // FALLBACK MODE: If it doesn't strictly match but looks like an ABTC file, try to map it anyway
                ...
                if (parsedData && parsedData.length > 0) {
                    ...
                    console.log("✅ Fallback parsed successful:", excelData.length, "municipalities compiled.");
                } else {
                    alert(`ERROR: Unrecognized format.\n\nDetected Headers: [${originalKeys.join(", ")}]\n\nPlease check your browser's console (F12) for detailed diagnostics.`);
                    return;
                }
            }
...
// ==============================================================================
// TRANSFORM PHO ABTC FORMAT (ENHANCED REMARK MATCHING)
// ==============================================================================
function transformPHOABTCFormat(phoData, originalKeys) {
    const municipalityMap = {};
    
    // Find the keys dynamically by examining their clean strings
    const abtcKey = originalKeys.find(k => k.trim().toLowerCase().includes('abtc') || k.trim().toLowerCase().includes('facility'));
    const totalKey = originalKeys.find(k => k.trim().toLowerCase() === 'total' || k.trim().toLowerCase().includes('cases') || k.trim().toLowerCase().includes('bite'));
    ...
    if (!abtcKey) {
        console.error("❌ Could not find a valid column representing the ABTC facility names.");
        return [];
    }
    ...
    phoData.forEach(function(row) {
        ...
        // Skip totals, headers, or null rows
        if (!facilityName || facilityName.toLowerCase() === 'total' || facilityName.toLowerCase() === 'nan' || facilityName.toLowerCase() === 'grand total') return;
        
        // Resolve facility coordinates and details
        const facilityInfo = getFacilityMunicipality(facilityName);
        if (!facilityInfo) {
            console.warn(`⚠️ Facility missing from reference mapping database: "${facilityName}"`);
            return;
        }
