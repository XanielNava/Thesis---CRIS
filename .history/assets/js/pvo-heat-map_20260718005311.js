...
document.addEventListener("DOMContentLoaded", async () => {
    initializeMap();
    initializeControls();
    fetchPopulationDataFromFirestore(); // Real-time listener handles its own state updates
...
// Real-time listener for population data
    ...
    try {
        ...
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            if (querySnapshot.empty) {
                ...
                updateHumanPopulationCard(); // Sync UI card state
                refreshMap(); // Refresh calculations on map statistics
                return;
            }
            ...
            querySnapshot.forEach((doc) => {
                ...
                if (municipalityName && popValue !== undefined) {
                    // Casing conversion to match raw case file data (e.g., "MINA" -> "mina")
            ...
            // Update global data structures securely
            ...
            // ✅ CRITICAL FIX: Push the new numbers to your UI DOM card instantly
            updateHumanPopulationCard();

            // ✅ CRITICAL FIX: Re-run statistics calculations so rates dynamically adjust
            refreshMap();
        });
