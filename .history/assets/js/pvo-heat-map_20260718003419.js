...
// Remove 'async' since onSnapshot is a real-time listener, not a one-time promise
function fetchPopulationDataFromFirestore() {
    try {
        console.log(" Attaching real-time listener to 'population_data'...");
        const populationCollection = collection(db, "population_data");
        
        // onSnapshot stays open and triggers this callback EVERY time data changes in Firestore
        onSnapshot(populationCollection, (querySnapshot) => {
            console.log(" FIRESTORE RESPONDED! Snapshot size:", querySnapshot.size);
            
            if (querySnapshot.empty) {
                console.warn(" population_data collection is empty.");
                totalHumanPopulation = 0;
                municipalityPopulations = {};
                // Trigger map update with empty data
                updateHeatmapLayer(); 
                return;
            }
            ...
            querySnapshot.forEach((doc) => {
                const data = doc.data();
                // Fixed the string syntax here
                console.log(`Document [${doc.id}]:`, data);
                
                // Assuming your documents look like: { name: "Oton", population: 50000 }
                // Adjust keys based on your actual Firestore fields
                if (data.municipality && data.population) {
                    updatedMunicipalities[data.municipality] = Number(data.population);
                    totalPopulation += Number(data.population);
                }
            });

            // 1. Update your global variables
            municipalityPopulations = updatedMunicipalities;
            totalHumanPopulation = totalPopulation;

            console.log("Updated Local Data. Total:", totalHumanPopulation);

            // 2. CRITICAL STEP: Tell your map engine to redraw right here!
            updateHeatmapLayer(); 
        });
...
// Separate function to handle the UI/Map re-render
function updateHeatmapLayer() {
    // Example: If you are using a heatmap layer instance, you have to refresh its data points
    if (typeof myHeatmapLayer !== 'undefined') {
        console.log("Refreshing map visualization...");
        
        // 1. Process your 'municipalityPopulations' into the coordinate array your map needs
        const newHeatmapPoints = transformPopDataToPoints(municipalityPopulations);
        
        // 2. Pass it to your layer (Syntax depends on Leaflet, Google Maps, etc.)
        // e.g., heatmapLayer.setData(newHeatmapPoints);
    }
}
// ==========================================================
...
function updateStatistics(data) {
    ...
    // Calculate active population for rate calculations (NOT for display)
    ...
    if (activePopulation === 0) {
        activePopulation = totalHumanPopulation > 0 ? totalHumanPopulation : 1500000;
    }

    // ✅ DON'T UPDATE THE HUMAN POPULATION CARD HERE - It's handled by updateHumanPopulationCard()
    // The card shows totalHumanPopulation, which is ALWAYS the total, not filtered

    // Calculate rates using activePopulation
    if (prevalenceCard) {
