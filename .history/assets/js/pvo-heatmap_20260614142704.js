/**
 * CRIS Heatmap Dashboard Handler Engine - Repaired Version
 */
...
// FORCE MAP TO RECALCULATE RE-RENDER SIZES IMMEDIATELY
setTimeout(() => {
    map.invalidateSize();
}, 300);
...
// Case-Insensitive Header Cleaner & Normalizer
function cleanHeaders() {
    excelData = excelData.map(function (row) {
        ...
        Object.keys(row).forEach(function (key) {
            ...
            // Normalize column variations to match uniform keys
            if (cleanKey === "year") newRow["Year"] = row[key];
            else if (cleanKey === "municipality" || cleanKey === "town") newRow["Municipality"] = row[key];
            else if (cleanKey === "latitude" || cleanKey === "lat") newRow["Latitude"] = row[key];
            else if (cleanKey === "longitude" || cleanKey === "lng") newRow["Longitude"] = row[key];
            else if (cleanKey.includes("bite")) newRow["Animal Bite Cases"] = row[key];
            else if (cleanKey.includes("human") && cleanKey.includes("death")) newRow["Human Rabies Deaths"] = row[key];
            else if (cleanKey.includes("animal") && cleanKey.includes("death")) newRow["Animal Rabies Deaths"] = row[key];
            else newRow[key.trim()] = row[key]; // Fallback
        });
        return newRow;
    });
}
...
function refreshMap() {
    ...
    // Recalculate dimensions dynamically on data modification events
    map.invalidateSize();
}
