...
function drawHeatmap(data) {
    ...
    data.forEach(function(row) {
        ...
        // Attach a unique, standalone circle shape element directly to this marker context
        marker.investigationZone = L.circle([lat, lng], {
            radius: 3000,           // 3000 meters = 3 Kilometers
            color: "#EA6113",       // CRIS Sunset Orange
            weight: 3,              // Thicker weight line border to pass through dark canvas layers
            fillColor: "#FBB931",   // CRIS Sunset Yellow
            fillOpacity: 0.18,      // Noticeable containment filling over maps
            dashArray: "6, 6"       // Delineated dashed inspection line formatting
        });

        // Stabilized structural action listeners to intercept infinite refresh loops
        marker.on('mouseover', function(e) {
            // Check if the layer isn't already rendering inside the active map layout pane
            if (!map.hasLayer(this.investigationZone)) {
                this.investigationZone.addTo(map);
                this.investigationZone.bringToFront();
            }
        ...
        marker.on('mouseout', function(e) {
            ...
            // Guard criteria: do not clear the circle if the mouse is moving directly into the popup container box
