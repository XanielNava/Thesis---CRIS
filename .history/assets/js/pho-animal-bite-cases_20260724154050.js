...
async function loadCases() {
    ...
    try {
        console.log("🔄 Loading cases from pho_rabies_cases collection...");
        
        // Simple getDocs without orderBy (no index required)
        const snapshot = await getDocs(collection(db, "pho_rabies_cases"));
        
        console.log(`📊 Firestore returned ${snapshot.size} documents`);
        
        if (snapshot.empty) {
            console.warn("⚠️ No cases found in pho_rabies_cases collection");
            allCases = [];
            filteredCases = [];
            renderTable();
            return;
