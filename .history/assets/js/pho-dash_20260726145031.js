...
// ================= FETCH HUMAN POPULATION DATA FROM FIRESTORE (SYNCED WITH HEATMAP) =================
...
async function loadHumanPopulation() {
    ...
    try {
        console.log("📊 Fetching human population data for Dashboard...");
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            // Check if this document is the master Provincial total record "ILOILO" / "ILOILO_TOTAL"
            if (docIdUpper === "ILOILO" || docIdUpper === "ILOILO_TOTAL") {
                totalPopulationAccumulator = Number(data.totalPopulation) || 0;
            } else {
                // Otherwise accumulate municipal totalPopulation
        ...
        // If no overall provincial total record was explicitly stored, use sum of municipalities
        if (totalPopulationAccumulator === 0) {
            totalPopulationAccumulator = sumOfMunicipalities;
        }

        const formattedVal = totalPopulationAccumulator.toLocaleString();
        popElem.textContent = formattedVal;
        console.log("✅ Dashboard Human Population updated to:", formattedVal);
