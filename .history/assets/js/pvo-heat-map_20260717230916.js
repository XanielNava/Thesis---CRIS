...
async function fetchPopulationDataFromFirestore() {
    try {
        ...
        // DEBUG TRIGGER 1: Check if Firestore returned an absolutely empty collection
        if (querySnapshot.empty) {
            console.warn("⚠️ 'population_data' collection returned 0 documents from Firestore.");
            if (populationCard) {
                populationCard.textContent = "Empty Collection";
            }
            return;
        }
        ...
        municipalityPopulations = {}; // Reset local cache

        querySnapshot.forEach((doc) => {
            const data = doc.data();
            
            // DEBUG TRIGGER 2: Look at the raw structure of the first document fetched
            if (municipalityCount === 0) {
                console.log("🔍 First Doc ID Found:", doc.id);
                console.log("🔍 First Doc Raw Fields:", JSON.stringify(data));
            }

            // Map the field definitions accurately regardless of naming conventions
        ...
        // Set global state tracking variable
        ...
        // DEBUG TRIGGER 3: Verify if the calculated total is hitting 0 due to field mismatches
        ...
        // DECOUPLED MANDATE: Force the display to show the total upload immediately.
        if (populationCard) {
...
async function fetchLatestCaseDataFromFirestore() {
    try {
        ...
        if (querySnapshot.empty) {
            ...
            // Explicitly evaluate stats with empty data to keep population card protected
            updateStatistics([]); 
            return;
        }
