...
async function fetchPopulationDataFromFirestore() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            // Normalize document ID or municipality field to lowercase for bulletproof matching
        ...
        console.log(`✅ Census Loaded: ${municipalityCount} municipalities. Total: ${totalHumanPopulation.toLocaleString()}`);
...
async function fetchLatestCaseDataFromFirestore() {
    try {
        ...
        if (excelData.length > 0) {
            populateFilters();
            refreshMap(); // This triggers updateStatistics() which links the population calculations together
        }
...
function updateStatistics(data) {
    ...
    // SAFEGUARD: If active population calculates to 0 (census hasn't loaded yet),
    // fallback to total human population, otherwise keep it clean.
    if (activePopulation === 0) {
        activePopulation = totalHumanPopulation > 0 ? totalHumanPopulation : 0;
    }

    // Render Human Population Card safely
...
function drawHeatmap(data) {
    ...
    // HEATMAP GRADIENTS (Strict decimal keys to align with Leaflet.heat parser)
...
function generateCluster(lat, lng, cases, spread) {
    ...
    // Guaranteed minimum of 1 point if cases > 0, scaling up with volume
