...
// Replace these placeholders with your actual credentials from your Firebase Console
const firebaseConfig = {
    apiKey: "YOUR_API_KEY",
...
// Initialize App and Firestore Database Connection
const app = initializeApp(firebaseConfig);
const db = getFirestore(app);
...
function renderCalendar(){
    ...
    for(let day = 1; day <= lastDate; day++){
        ...
        // Standardize date string to YYYY-MM-DD zero-padded format to match your database entries
        ...
        // Check Firestore database to see if a custom event exists to draw bottom indicators
        ...
        dateCell.addEventListener("click", async function(){
            ...
            // Reset textarea UI input box before fetching
            document.getElementById("eventInput").value = ""; 

            try {
                // Query collection 'calendar-events' filtering where field 'date' matches selection
...
// Check Firestore via structured queries to toggle highlighting states on your grid cells
async function checkAndMarkEvent(dateKey, cellElement) {
    try {
        ...
        console.error("Marker map scanning error: ", e);
    }
}
...
// ANNOUNCEMENT SAVE BUTTON ENGINE (FIREBASE ASYNC INTEGRATION)
document.getElementById("saveEventBtn").addEventListener("click", async function(){
    ...
    try {
        // Query to check if an announcement already exists for this specific day
        const q = query(collection(db, "calendar-events"), where("date", "==", selectedDate));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
            // Document already exists -> update matching entry field mapping data
            ...
            // No entry exists yet -> create a new document with an auto-generated random string ID
        ...
        alert("Announcement saved directly to CRIS Firebase cloud storage!");
        renderCalendar(); // Redraw UI elements cleanly to reflect updated graphic indicators
    } catch (error) {
        console.error("Error writing data matrix: ", error);
        alert("Database execution failed. Review connection constraints.");
    }
});
