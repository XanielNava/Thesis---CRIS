    ...
    try {
        ...
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            if (querySnapshot.empty) {
                ...
                if (typeof updateHeatmapLayer === "function") updateHeatmapLayer(); 
                return;
            }
            ...
            querySnapshot.forEach((doc) => {
                ...
                // Extracting fields exactly as they appear in your console snippet
                const municipalityName = data.municipality; 
                const popValue = data.totalPopulation;

                if (municipalityName && popValue !== undefined) {
                    // Normalize municipality names to standard casing if needed (e.g., "MINA")
                    const key = municipalityName.trim(); 
                    
                    // Convert to Number just to prevent any accidental string concatenation
                    const populationNum = Number(popValue);

                    updatedMunicipalities[key] = populationNum;
                    totalPopulationAccumulator += populationNum;
                }
            });

            // Update your global variables safely
            municipalityPopulations = updatedMunicipalities;
            totalHumanPopulation = totalPopulationAccumulator;

            console.log(" Current Local Population Object:", municipalityPopulations);
            console.log(" Combined Total Population:", totalHumanPopulation);

            // Trigger your map layer refresh logic here
            if (typeof updateHeatmapLayer === "function") {
                updateHeatmapLayer();
            } else {
                console.warn("updateHeatmapLayer function is not defined yet.");
            }
        });
