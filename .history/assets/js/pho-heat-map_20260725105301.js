...
// =========================================================
// BASE RADIUS CONFIGURATION - Keeps clusters same geographic size on zoom
// =========================================================
const BASE_RADIUS = {
    bite: 28,
    human: 24,
...
// =========================================================================
// CALCULATE SCALED RADIUS BASED ON ZOOM LEVEL (CORRECTED)
// =========================================================================
function getScaledRadius(baseRadius) {
    const currentZoom = map.getZoom();
    const zoomDiff = currentZoom - BASE_ZOOM;
    
    // Scale pixel radius UP as you zoom in so physical size on map remains constant
    const scaled = baseRadius * Math.pow(2, zoomDiff);
    
    // Bound the values to keep performance smooth at deep zoom levels
    return Math.min(Math.max(scaled, 8), 350);
}

// =========================================================================
// DRAW HEATMAP WITH ZOOM-AWARE RADIUS & STABLE INTENSITY (CORRECTED)
// =========================================================================
function drawHeatmap(data) {
    ...
    const currentZoom = map.getZoom();
    const calculatedRadius = getScaledRadius(BASE_RADIUS.bite);

    // Create heatmap layers with zoom-matched intensity and blur scaling
    biteHeatLayer = L.heatLayer(bitePoints, { 
        radius: calculatedRadius, 
        blur: Math.max(12, calculatedRadius * 0.6), 
        maxZoom: currentZoom, 
        minOpacity: 0.5, 
        max: 0.8, 
    ...
    humanHeatLayer = L.heatLayer(humanPoints, { 
        ...
        blur: Math.max(12, getScaledRadius(BASE_RADIUS.human) * 0.6), 
        maxZoom: currentZoom, 
        minOpacity: 0.5, 
        max: 0.8, 
    ...
    animalHeatLayer = L.heatLayer(animalPoints, { 
        ...
        blur: Math.max(12, getScaledRadius(BASE_RADIUS.animal) * 0.6), 
        maxZoom: currentZoom, 
        minOpacity: 0.5, 
        max: 0.8, 
    ...
    if (bounds.length > 0 && map.getZoom() === 9) {
