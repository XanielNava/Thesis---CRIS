...
async function fetchPopulationDataFromFirestore() {
    try {
        ...
        // Use onSnapshot instead of getDocs for real-time live sync
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            municipalityPopulations = {}; // Clear old memory before reload
            ...
            if (totalHumanPopulation === 0) {
                if (populationCard) populationCard.textContent = `Field Error (${municipalityCount} docs)`;
                return;
            }

            // Force update UI element instantly
            if (populationCard) {
            ...
            // If rabies case data is already active on the map, recalculate rates based on the new population numbers
        ...
        if (populationCard) populationCard.textContent = "Connection Error";
    }
}
