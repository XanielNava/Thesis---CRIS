...
async function fetchPopulationDataFromFirestore() {
    try {
        ...
        const populationCollection = collectio(db, "population_data");
        ...
        onSnapshot(populationCollection, (querySnapshot) => {
            ...
            console.log("✅ Total Human Population calculated:", totalHumanPopulation);
            
            // UPDATE THE HUMAN POPULATION CARD DIRECTLY
            updateHumanPopulationCard();
            
            // Refresh map to recalculate statistics with new population
            refreshMap();
        });
    } catch (error) {
        console.error("❌ Error in population listener:", error);
    }
}
