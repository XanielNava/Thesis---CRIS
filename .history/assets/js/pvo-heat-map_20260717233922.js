...
function updateStatistics(data) {
    ...
    if (municipalityFilter && municipalityFilter.value !== "All") {
        ...
        // Sum pop metrics matching the filter subset
        trackedMunicipalitiesInFilter.forEach(mun => {
            activePopulation += municipalityPopulations[mun] || 0;
        });
    }

    // Default back to global dataset if specific filter bounds read 0
    ...
    if (populationCard) {
        populationCard.textContent = activePopulation > 0 ? activePopulation.toLocaleString() : "0";
    }
