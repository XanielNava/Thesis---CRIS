...
// Replace these placeholders with actual values from your Firebase Project Console Settings
const firebaseConfig = {
    apiKey: "YOUR_API_KEY",
...
function renderCalendar(){
    ...
    for(let day = 1; day <= lastDate; day++){
        ...
        // Create standard YYYY-MM-DD Key string format
        const dateKeyString = `${year}-${month + 1}-${day}`;
        ...
        // Real-time visual cue if document exists in Firestore database
        ...
        dateCell.addEventListener("click", async function(){
            ...
            // Automatically load text entries from firestore database when clicked
            document.getElementById("eventInput").value = ""; 
            try {
                const docRef = doc(db, "announcements", selectedDate);
                const docSnap = await getDoc(docRef);
                
                if (docSnap.exists()) {
                    document.getElementById("eventInput").value = docSnap.data().event || "";
                }
            } catch (error) {
...
// Check Firestore to see if date contains saved information
async function checkAndMarkEvent(dateKey, cellElement) {
    try {
        const docRef = doc(db, "announcements", dateKey);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists() && docSnap.data().event.trim() !== "") {
            // Adds a marker style for days containing content
            cellElement.classList.add("has-event");
        }
    } catch (e) {
        // Suppress initial rules errors on clean launch configurations
    }
}
...
document.getElementById("saveEventBtn").addEventListener("click", async function(){
    ...
    try {
        // Point doc name path cleanly to specific isolated unique date string
        const docRef = doc(db, "announcements", selectedDate);
        await setDoc(docRef, {
            date: selectedDate,
            event: text,
            updatedBy: "Xaniel Nava",
            timestamp: new Date()
        });

        alert("Announcement saved directly to Firebase cloud storage!");
        renderCalendar(); // Rerender cleanly to instantly apply graphic dots
    } catch (error) {
