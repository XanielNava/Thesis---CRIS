...
/* ================================
   ROWS PER PAGE SELECTOR INITIALIZER
================================ */
document.getElementById("rowsPerPageSelect").addEventListener("change", (e) => {
    rowsPerPage = parseInt(e.target.value, 10);
    currentPage = 1; // Always return to Page 1 when modifying dataset scale
    renderTable();
});

/* ================================
   LOAD DATA WITH POLISHED SPINNER
================================ */
            ...
            <tr>
                <td colspan="7">
                    <div class="table-placeholder error">
                        ...
                        <p style="font-size:12px; color:#888; margin-top:4px;">Please check network connection links or cluster security policies.</p>
                    </div>
                </td>
            </tr>
        `;
...
/* ================================
   RENDER TABLE WITH SUMMARY AGGREGATIONS
================================ */
function renderTable() {
            ...
            <tr>
                <td colspan="7">
                    <div class="table-placeholder">
                        ...
                        <p style="font-size:12px; color:#888; margin-top:4px;">Your filter query returned zero matches against existing registry tracking indexes.</p>
                    </div>
                </td>
            </tr>
        `;
    ...
    // Render Paginated Data Intersect Rows
    ...
    // Running Filter Total Computations
    let sumMale = 0, sumFemale = 0, sumCat1 = 0, sumCat2 = 0, sumCat3 = 0;
    filteredCases.forEach(c => {
        sumMale += Number(c.male ?? 0);
        sumFemale += Number(c.female ?? 0);
        sumCat1 += Number(c.category1 ?? 0);
        sumCat2 += Number(c.category2 ?? 0);
        sumCat3 += Number(c.category3 ?? 0);
    });

    tbody.innerHTML += `
        <tr style="background-color: #FFE3B3; font-weight: bold; border-top: 2px solid #EA6113; position: sticky; bottom: 0; z-index: 5;">
            <td>TOTAL (Filtered Ledger Summary)</td>
            <td>${sumMale}</td>
            <td>${sumFemale}</td>
            <td>—</td>
            <td>${sumCat1}</td>
...
document.getElementById("caseSearch").addEventListener("input", (e) => {
    ...
    filteredCases = allCases.filter(caseData => {
        ...
        const ageField = String(caseData.age ?? "").toLowerCase();
        return abtcField.includes(query) || ageField.includes(query);
    });

    currentPage = 1; // Lock to Page 1 immediately upon text ingestion
    renderTable();
});

/* ================================
   PAGINATION BLOCK ENGINE WITH SMART ELLIPSES (...)
================================ */
...
document.getElementById("excelBtn").addEventListener("click", () => {
    ...
    const dataRows = filteredCases.map(c => [
        c.abtc ?? "", c.male ?? 0, c.female ?? 0, c.age ?? "", c.category1 ?? 0, c.category2 ?? 0, c.category3 ?? 0
    ]);
...
document.getElementById("pdfBtn").addEventListener("click", () => {
    ...
    const headers = [["ABTC", "Male", "Female", "Age", "Cat 1", "Cat 2", "Cat 3"]];
...
// Print Preview Window Engine Hook
document.getElementById("printBtn").addEventListener("click", () => {
    ...
    win.document.write(`
        ...
        <body>
            ...
            <table>
                <thead>
                    <tr><th>ABTC</th><th>Male</th><th>Female</th><th>Age</th><th>Category 1</th><th>Category 2</th><th>Category 3</th></tr>
                </thead>
