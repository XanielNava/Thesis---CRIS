...
/**
 * Purges all existing documents in pvo_rabies_cases
 */
async function clearCurrentDataset() {
    ...
    querySnapshot.forEach((documentSnap) => {
        batch.delete(doc(db, "pvo_rabies_cases", documentSnap.id));
    });
...
export async function uploadCasesDataset(parsedExcelRows, mode = "replace") {
    try {
        ...
        parsedExcelRows.forEach((row, index) => {
            const municipalityName = String(row["Mun/City"] || row.municipality || row["Municipality"] || "").trim();
        ...
        // Reload table view with freshly updated data
        await loadCases();
...
/**
 * Triggers confirmation prompt asking user for Replace vs Merge choice
...
export async function handleFileUploadPrompt(parsedExcelRows) {
    ...
    // If database already contains records, ask user for mode choice
    if (!existingSnapshot.empty) {
        ...
        // No existing records found, default to direct upload
...
async function loadCases() {
    ...
    try {
        ...
        // Sort by documentOrder to preserve original upload order
