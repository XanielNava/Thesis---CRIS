...
function updateStatistics(data) {
    ...
    /**
     * PHO LEADER FRAMEWORK ALIGNMENT
     * Formulate Prevalence Rate using the confirmed 1.5 Million metric baseline multiplier.
     */
    ...
    if (elPrev) elPrev.textContent = prevalenceRate.toFixed(2); // String format adjusted out of percentage display rules
...
function drawHeatmap(data) {
    ...
    data.forEach(function (row) {
        ...
        // Popups data mapping calculations using verified PHO 1.5M multiplier
        ...
        // Instantiates a precise 3-kilometer (3000 meter) operational quarantine boundary
        const investigationZone = L.circle([lat, lng], {
            radius: 3000,           // 3000 meters = 3 Kilometers radius
            color: "#EA6113",       // Sunset Orange perimeter color token
            weight: 2,              // Made slightly thicker for clear visibility over heat layers
            fillColor: "#FBB931",   // Sunset Yellow zone fill token
            fillOpacity: 0.15,      // Opacity adjusted to show on top of heavy canvas clusters
            dashArray: "6, 6"       // Delineated dash formatting denoting containment investigation lines
        });

        // Interactive dynamic hover attachment pipeline
        marker.on('mouseover', function () {
            // FIX: Add directly to markerLayer group to force it to render on top of the canvas
            investigationZone.addTo(markerLayer);

            // Safety fallback: explicit execution layer bump
            investigationZone.bringToFront();

            this.openPopup();
        });

        marker.on('mouseout', function () {
            // FIX: Clean it out of the markerLayer group accurately
            markerLayer.removeLayer(investigationZone);
