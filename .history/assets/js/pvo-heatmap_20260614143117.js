/**
 * CRIS Heatmap Dashboard Handler Engine - Instant Initial Load
 */

// 1. MAP INITIALIZATION CONFIGURATIONS (Runs instantly on page load)
const map = L.map("map", { 
    zoomControl: true,
    minZoom: 2
}).setView([10.90, 122.60], 9); // Centers on Iloilo province coordinates immediately
...
// CRITICAL TIMEOUT: Forces the browser to draw the map tiles instantly on load
setTimeout(() => {
    map.invalidateSize();
}, 250);
