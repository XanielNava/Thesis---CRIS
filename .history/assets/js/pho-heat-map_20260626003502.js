...
async function loadPopulationData() {
    try {
        // 1. Fetch your official PSA file directly from your assets folder
        ...
        // 3. Convert sheet to raw array matrices
        const rawRows = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
        
        const parsedPopulation = [];

        // 4. Parse rows to extract only Municipality total rows matching PSA layout rules
        rawRows.forEach(function(row) {
            if (!row || row.length < 3) return;

            const nameValue = String(row[0]).trim();
            const totalPopulationValue = Number(row[2]); // Total population resides in index column 2 (C)

            // PSA structural filter: Find rows where the geographical name is in ALL CAPS
            // Excluding metadata strings like 'TABLE', 'PROVINCE', or empty data rows
            if (
                nameValue && 
                nameValue !== "undefined" &&
                ...
                // Clean structural names (e.g., stripping asterisks or text prefixes like 'CITY OF ')
