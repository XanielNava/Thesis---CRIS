...
// ==========================================================
// FETCH POPULATION DATA FROM IMPORT_HISTORY
// ==========================================================

async function fetchPopulationDataFromFirestore() {
    try {
        console.log("👥 Fetching population data from import_history...");
        
        // Query to get the latest import document (ordered by timestamp descending)
        const q = query(
        ...
        if (querySnapshot.empty) {
            console.warn("⚠️ No import data found in import_history yet.");
            return;
        }

        const latestImport = querySnapshot.docs[0].data();
        console.log("✅ Latest import document retrieved:", latestImport);

        // Extract total population from the latest import
        // totalCases contains the population total from population data uploads
        const totalPopulation = Number(latestImport.totalCases) || 0;
        ...
        console.error("❌ Error loading population data from import_history:", error);
    }
}
