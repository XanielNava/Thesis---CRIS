...
async function loadCases() {
    ...
    try {
        ...
        snapshot.forEach((doc) => {
            ...
            console.log(`📄 Processing document: ${data.municipality || "Unknown"}`);
            
            allCases.push({
                id: doc.id,
                
                // District & Municipality (Primary identifiers)
...
function renderTable() {
    ...
    const avgVaccinationPercentage = filteredCases.length > 0 
        ? (totals.vaccinationPercentage / filteredCases.length).toFixed(2)
        : 0;
...
/* STICKY HEADER OFFSET SYNC
   The CSS ships a 44px fallback for the second header row's "top" value,
   but real row height depends on font rendering and how much header text
   wraps, so it's measured here and applied exactly. Re-run any time the
   table reflows or the window resizes. */
