...
async function loadHumanPopulation() {
    ...
    try {
        ...
        console.log("Documents retrieved from pho_population_data:", querySnapshot.size);

        let totalPopulation = 0;

        querySnapshot.forEach((docSnap) => {
            const data = docSnap.data();
            console.log("Document Content:", docSnap.id, data);

            // Checks common population key names stored in Firestore
            const rawVal = data.population ?? data.count ?? data.iloiloPopulation ?? data.value ?? 0;
            
            // Strips commas if the database saved formatted string numbers like "2,050,000"
