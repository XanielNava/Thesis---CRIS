...
// ================= INTERFACE & DATABASE DATA COUPLING =================
// Replaced relative module point to use your central firebase module
...
async function loadHumanPopulation() {
    ...
    try {
        // Fetches directly from your Firestore 'iloilo-population' collection
        const querySnapshot = await getDocs(collection(db, "iloilo-population"));
        let totalPopulation = 0;

        querySnapshot.forEach((docSnap) => {
            const data = docSnap.data();
            totalPopulation += Number(data.population || data.count || 0);
        });

        // Displays formatted total (e.g., 2,050,000)
        popElem.textContent = totalPopulation.toLocaleString();

    } catch (error) {
        console.error("Error fetching Human Population: ", error);
        
        // Fallback check if settings saved data in localStorage
        const savedData = localStorage.getItem('cris_dashboard_data');
        if (savedData) {
            const parsed = JSON.parse(savedData);
            if (parsed.iloiloPopulation) {
                const total = parsed.iloiloPopulation.reduce((sum, item) => {
                    const val = typeof item === 'object' ? (item.population || 0) : item;
                    return sum + Number(val || 0);
                }, 0);
                popElem.textContent = total.toLocaleString();
            }
        }
    }
}

// Watch for changes if updated in real-time from settings
window.addEventListener('storage', (event) => {
    if (event.key === 'cris_dashboard_data') {
        loadHumanPopulation();
    }
});

// Run chart initialization AND population fetch on startup
document.addEventListener('DOMContentLoaded', () => {
    initDashboardChart();
    loadHumanPopulation(); // <--- Added here
});
