...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleProcessUpload() {
        ...
        try {
            ...
            // References to the targeted collections
            const populationColRef = collection(db, "pho_population_data");
            const historyColRef = collection(db, "pho_import_history");

            // Capture exact current local time
            const now = new Date();

            // 1. Queue write actions for individual municipality population datasets
            parsedRows.forEach(row => {
                const docRef = doc(populationColRef);
                batch.set(docRef, {
            ...
            // Double check values are locked in before committing
            const logCases = calculatedTotalPopulation;
            const logFacilities = totalMunicipalities;
            const logFileName = selectedFile.name;

            const logRef = doc(historyColRef);
            batch.set(logRef, {
                fileName: logFileName,
                totalCases: logCases,         // Mapped so settings.js reads it under (data.totalCases)
                totalFacilities: logFacilities, // Mapped so settings.js reads it under (data.totalFacilities)
                timestamp: now,
                status: "Completed"           // Expressly set as "Completed"
            });

            console.log(`📤 [CRIS] Writing ${parsedRows.length} population documents to Firestore...`);
            await batch.commit();
            ...
            alert(`Success! Iloilo Population Dataset imported.\n\nFile: ${logFileName}\nTotal Population Logged: ${logCases.toLocaleString()}\nMunicipalities: ${logFacilities}`);
