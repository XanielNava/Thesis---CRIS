    ...
    function handleFileSelection(e) {
        ...
        reader.onload = function(evt) {
            try {
                ...
                // ===== PARSE DATA ROWS (Skip 3 header rows, start from row 4 = index 3) =====
                const cleanData = [];
                ...
                for (let rowIdx = 3; rowIdx < rawRows.length; rowIdx++) {
                    ...
                    // These are the fixed column positions in the PSA format:
                    cleanData.push({
                        abtc: facilityName,
                        maleCases: Number(row[1]) || 0,                  // Col B: Male
                        femaleCases: Number(row[2]) || 0,                // Col C: Female
                        ageLt15: Number(row[3]) || 0,                    // Col D: < 15
                        ageGt15: Number(row[4]) || 0,                    // Col E: > 15
                        bitingDog: Number(row[5]) || 0,                  // Col F: Dog
                        bitingCat: Number(row[6]) || 0,                  // Col G: Cat
                        bitingOthers: Number(row[7]) || 0,               // Col H: Others
                        humanCat1: Number(row[8]) || 0,                  // Col I: Cat 1
                        humanCat2: Number(row[9]) || 0,                  // Col J: Cat 2
                        humanCatNew: Number(row[10]) || 0,               // Col K: Cat New
                        humanCatBooster: Number(row[11]) || 0,           // Col L: Cat Booster
                        hr: Number(row[12]) || 0,                        // Col M: HR
                        petTcv: Number(row[13]) || 0,                    // Col N: TCV
                        petHrig: Number(row[14]) || 0,                   // Col O: HRIG
                        petErig: Number(row[15]) || 0,                   // Col P: ERIG
                        total: Number(row[16]) || 0,                     // Col Q: TOTAL
                        remarksCompII: Number(row[17]) || 0,             // Col R: Comp II
                        remarksCompIII: Number(row[18]) || 0,            // Col S: Comp III
                        remarksIncompleteII: Number(row[19]) || 0,       // Col T: Incomplete II
                        remarksIncompleteIII: Number(row[20]) || 0,      // Col U: Incomplete III
                        remarksNoneII: Number(row[21]) || 0,             // Col V: None II
                        remarksNoneIII: Number(row[22]) || 0,            // Col W: None III
                        rep: Number(row[23]) || 0                        // Col X: REP
                    });
                }
