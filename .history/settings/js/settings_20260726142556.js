...
const HISTORY_CACHE_TIME = 5 * 60 * 1000; // 5 minutes
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Load initial 15 history logs on page load (WITH CACHING)
    loadImportHistory();
    ...
    // -------------------------------------------------------------
    if (populationForm) {
        const populationHandler = async (e) => {
            e.preventDefault();
        ...
        populationForm.addEventListener("submit", populationHandler);
        listeners.push(() => populationForm.removeEventListener("submit", populationHandler));
    }
    ...
    if (legacyForm) {
        ...
        legacyForm.addEventListener("submit", legacyHandler);
        listeners.push(() => legacyForm.removeEventListener("submit", legacyHandler));
    }
    ...
    /**
     * Load import history with caching to prevent Firebase quota exhaustion
     * Caches results for 5 minutes before querying again
     */
    async function loadImportHistory() {
        if (!historyTableBody) return;
        
        const now = Date.now();
        const timeSinceLastLoad = now - lastHistoryLoad;

        // Use cache if loaded within the last 5 minutes
        if (timeSinceLastLoad < HISTORY_CACHE_TIME) {
            console.log(`📦 Using cached history (${Math.round((HISTORY_CACHE_TIME - timeSinceLastLoad) / 1000)}s remaining)`);
            return;
        }

        try {
            console.log("🔄 Fetching fresh import history from Firebase...");
