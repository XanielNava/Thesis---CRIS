...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // -------------------------------------------------------------
    // 2. HEALTH FACILITY MANAGEMENT (ABTC/ABCC DIRECTORY)
    // -------------------------------------------------------------
    const saveManagedFacilityBtn = document.getElementById("saveManagedFacilityBtn");
    const cancelFacilityEditBtn = document.getElementById("cancelFacilityEditBtn");
    ...
    if (saveManagedFacilityBtn) {
        saveManagedFacilityBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                saveManagedFacilityBtn.innerText = editingFacilityId ? "Update Facility" : "Add Facility";
            }
        });
    }

    if (cancelFacilityEditBtn) {
        cancelFacilityEditBtn.addEventListener("click", () => {
            resetFacilityForm();
        });
    }
...
// Helper: Load Managed Facilities Directory
async function loadManagedFacilities() {
    ...
    try {
        ...
        if (snap.empty) {
            tableBody.innerHTML = `<tr><td colspan="6" class="text-center text-muted">No operational health facilities registered yet.</td></tr>`;
            return;
        }
...
function resetFacilityForm() {
    ...
    const saveBtn = document.getElementById("saveManagedFacilityBtn");
    const cancelBtn = document.getElementById("cancelFacilityEditBtn");
    if (saveBtn) saveBtn.innerText = "Add Facility";
