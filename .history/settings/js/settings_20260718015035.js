...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetToEmptyState() {
        ...
        if (fileInput) {
            fileInput.value = null; // Clear old values safely
        }
    ...
    async function handleProcessUpload() {
        ...
        try {
            ...
            // If we have remaining operations or room left in our current batch, write history log inside it
            if (opCount >= batchLimit) {
                await batch.commit();
                batch = writeBatch(db);
            }
            ...
            console.log(`📤 [CRIS] Uploading final batch details containing historical log logs...`);
            await batch.commit();
            ...
            if (parsedData.maleKey && parsedData.femaleKey) {
                alertDetails += `Calculated Breakdown:\n` +
                    `Male Patients: ${parsedData.maleCasesCount.toLocaleString()}\n` +
                    `Female Patients: ${parsedData.femaleCasesCount.toLocaleString()}\n`;
            }
    ...
    // =========================================================================
    // 👥 ADDED: POPULATION DATA CARD MANAGEMENT (INTEGRATED SINGLE-DOC SCANNER)
    // =========================================================================
    ...
    if (popCardElement) {
        const popFileInput = popCardElement.querySelector('#popDataFile') || popCardElement.querySelector('input[type="file"]');
        ...
        function handlePopFileSelection(e) {
            ...
            reader.onload = function(evt) {
                try {
                    ...
                    let overallProvincePopulation = 0;

                    for (let i = 0; i < rawRows.length; i++) {
                        const row = rawRows[i];
                        if (!row || row.length < 3) continue;

                        const nameValue = row[0] ? row[0].toString().trim() : "";
                        
                        if (nameValue.toUpperCase().includes("ILOILO *") || nameValue.toUpperCase() === "ILOILO *") {
                            overallProvincePopulation = Number(row[2]) || 0; 
                            break; 
                        }
                    }

                    if (overallProvincePopulation === 0) {
                        throw new Error("Could not find the 'ILOILO *' master total population row in this file.");
                    }

                    popSelectedFile = file;
                    popParsedData = { 
                        municipalityId: "ILOILO_TOTAL", 
                        totalPopulation: overallProvincePopulation 
                    };

                    popUploadLabel.innerHTML = `
                        <div style="padding: 10px; background: #e3f2fd; color:#0d47a1; border-radius:6px; font-size: 13px;">
                            Selected: <b>${file.name}</b> <br>
                            Master Total Detected: <b>${overallProvincePopulation.toLocaleString()}</b>
                        </div>`;
                } catch (err) {
        ...
        async function handleProcessPopUpload() {
            ...
            popUploadBtn.disabled = true; popUploadBtn.textContent = 'Saving...';

            try {
                let batch = writeBatch(db);
                const docRef = doc(collection(db, "population_data"), popParsedData.municipalityId);
                
                batch.set(docRef, {
                    municipality: popParsedData.municipalityId, 
                    totalPopulation: popParsedData.totalPopulation,
                    uploadedAt: new Date()
                });
                ...
                await batch.commit();
                addHistoryRow(new Date().toLocaleString(), popSelectedFile.name, `Master Population Updated`, 'Completed');
                alert(`✅ Success! Master population of ${popParsedData.totalPopulation.toLocaleString()} uploaded to Firestore.`);
